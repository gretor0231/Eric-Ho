...
#define TRUE 1
#define FALSE 0
#define UNSINGED_INT_MAX 4294967295
/** CS241 LAB3
 * Cheng En Ho
 * Jan.30 2018*/
...
unsigned int getbits(unsigned long x, int p, int n){
...
void pgetbits(unsigned long x, int p, int n){
    ...
    int state;
        ...
        if(n - p > 1){
            ...
            state = FALSE;
        }else if(x > UNSINGED_INT_MAX ){/* The unsigned int max range is 4294967295.*/
            printf("Error: value out of range\n");
            state = FALSE;
        }

        if(state){
            printf("getbits(x=%lu, p=%d, n=%d) = %u\n",x,p,n,getbits(x, p, n));
    }
}
